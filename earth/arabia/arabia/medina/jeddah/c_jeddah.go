...
type JeddahCounty interface {
    feud.County
    BAl_lith利斯() 	feud.Barony
    BAs_sirrayn锡赖因() 	feud.Barony
...
type 吉达JeddahCounty struct {
	feud.BaseCounty
	利斯Al_lith 	feud.Barony
	锡赖因As_sirrayn 	feud.Barony
...
func (c *吉达JeddahCounty) BAl_lith利斯() feud.Barony {
	return c.利斯Al_lith
}
    
func (c *吉达JeddahCounty) BAs_sirrayn锡赖因() feud.Barony {
	return c.锡赖因As_sirrayn
}
...
func init() {
	...
	f.利斯Al_lith = BAl_lith利斯
	f.利斯Al_lith.SetParent(f)
	
	f.锡赖因As_sirrayn = BAs_sirrayn锡赖因
	f.锡赖因As_sirrayn.SetParent(f)
